...
class AuthController {
    ...
    async login(req: Request, res: Response) {
        try {
            ...
            const result = await make_request_to_db(
                'SELECT Users.*, Passwords.password FROM users JOIN Passwords ON Users.password_id = Passwords.id WHERE Users.email = $1',
                [email],
            )
    ...
    async me(req: Request, res: Response) {
        try {
            ...
            const { id, name, surname, email } = result.rows[0]
            res.status(200).json({ id, name, surname, email })
        } catch (err) {
            console.error(err)
            res.status(500).json({ message: 'Server error' })
        }
